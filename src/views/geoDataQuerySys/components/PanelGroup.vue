...
export default {
  ...
  data() {
    return {
      cols: [
				{title: '空气温度传感器', fixed: 2, nowReq: 'get_T1', svg: 'temp', format: 'jpg'},
				{title: '土壤温湿度传感器', fixed: 4, nowReq: 'get_Humidity1_mean', svg: 'humidity', format: 'jpg'},
				{title: '孔隙压传感器', fixed: 3, nowReq: 'get_Pressure1_mean', svg: 'kong', format: 'png'},
				// {title: '加速度', fixed: 4, nowReq: 'get_A1_variance', svg: 'Acc'},
				{title: '雨量计', fixed: 4, nowReq: 'get_rainfall_mean', svg: 'rain', format: 'jpg'},
				// {title: '光照度', fixed: 0, nowReq: 'get_illuminance_mean', svg: 'illuminance'},
				{title: '微震监测计', fixed: 0, nowReq: 'get_illuminance_mean', svg: 'zhen', format: 'jpg'},
				{title: '不极化电极', fixed: 0, nowReq: 'get_V1_mean', svg: 'dian', format: 'jpg'},
        {title: 'GNSS位移检测仪', fixed: 1, nowReq: 'get_displacement1_mean', svg: 'GNSS', format: 'png'},
        {title: '拉线式位移计', fixed: 1, nowReq: 'get_displacement1_mean', svg: 'line', format: 'png'},
        {title: '大气压传感器', fixed: 1, nowReq: 'get_displacement1_mean', svg: 'air', format: 'jpg'},
      ],
      nowReqObj: {},
      newData: {},
      nowBaseData: [],
  ...
  methods: {
    showDlg({nowReq, fixed, title, svg, format}){
      api[nowReq]({hour: 1}).then((data) => {
        this.showDetailDlg = true
        this.$nextTick(() => {
          this.nowReqObj = {data, fixed, title, svg, format}
        })
      })
    },
    queryData(req, fixed){
